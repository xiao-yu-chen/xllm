...
| `cp_size` | `int32` | `1` | DSA attention 的 context parallel 规模。 |
...
| `enable_prefill_sp` | `bool` | `false` | 是否启用仅 prefill 阶段的 sequence parallel；支持 `enable_chunked_prefill=true`，但仅限纯 prefill batch（`PREFILL` / `CHUNKED_PREFILL`）。 |
